...
/**
 * Common fiscal view for Tax, Hdr and Fsk schemas
 */
var FiscView = PageView.extend({

    /**
     * HTML template
     */
    template: _.template($('#fisc-tmpl').html()),

    /**
     * Name of the schema in the view
     */
    schemaName: '',
     ...
     */
    events: {
        'click .btn-fisc-save': 'saveData'
    },

    /**
     * Handler which sends request
     */
    ...
    /**
     * Render the view due to the passed schema name
     * @returns {FiscView}
     */
    render: function () {
        var self = this;
        ...
        switch (this.schemaName) {
            case 'Tax':
                ...
                label = t("Put taxes to fiscal memory");
                self.functionToExecute = self.doTax;
                break;
            case 'Fsk':
                this.table = new TableContainer({
                ...
                label = t("Fiscalize");
                self.functionToExecute = self.doFisc;
                break;
            case 'Hdr':
                this.table = new TableContainer({
                ...
                label = t("Put headers to fiscal memory");
                self.functionToExecute = self.doHdr;
                break;
            default:
                break;
        }
        ...
        if (this.schemaName == 'Fsk' || fiscalCell.get("fiscalize")) {
     ...
     * @param button
     */
    sendRequest: function (url, data, button) {
        var self = this;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response && response.err) {
                    var message = schema.error(response.err);
                    self.onErrorEvent(message);
                }
                else {
                    self.onInfoEvent(t("Changes were applied!"))
                }
            },
            error: function () {
    ...
    /**
     * Send request to put taxes
     * @param e
     */
    doTax: function (e) {
        this.sendRequest('/cgi/proc/puttaxfm', {}, e.target);
        //console.log('Save Tax');
    },
    /**
     * Send request to make fiscalization
     * @param e
     */
    doFisc: function (e) {
        this.sendRequest('/cgi/proc/fiscalization', {}, e.target);
    },
    /**
     * Send request to put headers
     * @param e
     */
    doHdr: function (e) {
        this.sendRequest('/cgi/proc/puthdrfm', {}, e.target);
    },
    /**
...
var FiscDo = FiscView.extend({
    events: {
        'click #fsc': 'fiscalize'
    },
    remove: function () {
        this.fsk.remove();
        FiscView.prototype.remove.call(this);
    },
    render: function () {
 ...
 * Certificate block
 */
var CertificateBlock = PageView.extend({
    /**
...
/**
 * Electronic journal page
 */
var ElectronicJournalPage = PageView.extend({
     ...
     * Template
     */
    template: _.template($("#electronic-journal-tmpl").html()),
    ...
    /**
     * Variable to check if the view is rendered at the first time
     */
    initProcess: true,

    /**
     * Render function
     * @returns {ElectronicJournalPage}
     */
    render: function () {
        var self = this;
        this.delegateEvents();
